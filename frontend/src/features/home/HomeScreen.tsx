...
export function HomeScreen() {
  ...
  const { recentlyExplored, isLoading: isCareerExplorationLoading } = useCareerExplorationContext();
  const { decisionMemory, isMemoryLoading } = useDecisionContext();
  const { items: historyItems, isLoading: isHistoryLoading } = useHistoryContext();

  // Sprint 9 — these are the same 5 "immediate" Sprint-5 providers Mission
  // Control has always read from; the only change is honestly reflecting
  // that they're asynchronous, so a returning student with real data never
  // briefly sees "Nothing yet"/an empty Recent Investigations list before
  // it arrives.
  const isLoading = isDiscoveryLoading || isCareerExplorationLoading || isMemoryLoading || isHistoryLoading;
